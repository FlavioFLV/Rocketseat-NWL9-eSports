        ...
        < Dialog.Portal >
            ...
            <Dialog.Content className='fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-black/25'>
                ...
                <form onSubmit={handleCreateAd} className='mt-8 flex flex-col gap-4'>
                    ...
                    <div className='flex flex-col gap-2'>
                        <label htmlFor="name">Seu nome (ou nickename)?</label>
                    ...
                    <div className='flex gap-6'>
                        <div className='flex flex-col gap-2'>
                            ...
                            <ToggleGroup.Root type='multiple' className='grid grid-cols-4 gap-2' value={weekDays} onValueChange={setWeekDays}>
                                ...
                                <ToggleGroup.Item value="1" title='Segunda-Feira' className={`w-8 h-8 rounded ${weekDays.includes('1') ? 'bg-violet-500' : 'bg-zinc-900'}`}>S</ToggleGroup.Item>
                                <ToggleGroup.Item value="2" title='Terça-Feira' className={`w-8 h-8 rounded ${weekDays.includes('2') ? 'bg-violet-500' : 'bg-zinc-900'}`}>T</ToggleGroup.Item>
                                <ToggleGroup.Item value="3" title='Quarta-Feira' className={`w-8 h-8 rounded ${weekDays.includes('3') ? 'bg-violet-500' : 'bg-zinc-900'}`}>Q</ToggleGroup.Item>
                                <ToggleGroup.Item value="4" title='Quinta-Feira' className={`w-8 h-8 rounded ${weekDays.includes('4') ? 'bg-violet-500' : 'bg-zinc-900'}`}>Q</ToggleGroup.Item>
                                <ToggleGroup.Item value="5" title='Sexta-Feira' className={`w-8 h-8 rounded ${weekDays.includes('5') ? 'bg-violet-500' : 'bg-zinc-900'}`}>S</ToggleGroup.Item>
                    ...
                    <label className='mt-2 flex items-center gap-2 text-sm'>
                        <Checkbox.Root
                            ...
                            onCheckedChange={(checked) => {
                                return checked ? setUseVoiceChannel(true) : setUseVoiceChannel(false)
                            }}
